...
(() => {
  const CONSTS = {
    ...
    STAR_MATRIX: [[1, 2, 3, 4, 5, 6, 5, 4, 3, 2], [3, 4, 5, 6, 5, 4, 3, 2, 1, 2], [5, 6, 5, 4, 3, 2, 1, 2, 3, 4]]
  };
  ...
  const Message = VirtualBandung.message = new class {
    ...
    setLinkButton(link, buttonName) {
      ...
      this.button.on('click', () => window.open(link)).on('pointerover', () => {
  ...
  class Building {
    constructor(imageInfo, xIndex, zIndex, link, button, messageId, messageJp) {
      ...
      sprite.on('pointerover', () => this.onPointerOver()).on('pointerout', () => this.onPointerOut()); // .on('click', () => this.onClick())
      // .on('tap', () => this.onClick());
    }
    ...
    tick(ticker) {// const countUnit = 30;
  ...
  const Buildings = VirtualBandung.Play.buildings = new class {
    ...
    init() {
      this._buildings = [[images.building1, 1.5, 'https://worldscuad.com/', 'LEARN MORE', `SCUAD terlahir dari ide dua orang perempuan yang bertemu di Tokyo. Mereka memiliki visi yang sama untuk menghubungkan anak-anak muda lintas negara dalam berkontribusi ide dan kreativitas, untuk menyelesaikan masalah sosial dan budaya serta menjadikan dunia yang lebih baik.`, `SCUADのはじまりは東京で出会った2人の女性たちのアイディア。国を越えて同世代の若い人たちを繋ぎ、ソーシャルアクションの分野とカルチャーの分野も繋いでみんなでクリエイティブなものごとを生み出せたら素敵な世界をつくれるはず、と同じ夢を描いて動き出しました。`], [images.building2, -1.5, 'https://www.instagram.com/ppikanto/', 'KNOW MORE', `SCUAD kini telah berkembang menjadi sebuah grup besar yang terdiri dari beragam orang-orang yang berbagi energi dan kreativitas yang sama. Acara SCUAD dapat terlaksana dengan adanya bantuan dari PPI Kanto. Jika kamu tertarik dengan kuliah di Jepang, kunjungi dan follow akun Instagram @ppikanto!`, `気づけばSCUADは成長し、同じ志と高いクリエイティビティを持つ様々な人たちが集まる大きなチームになりました。このイベントはインドネシア留学生協会関東支部（PPI Kanto）を運営パートナーに迎えたことで実現しています。`], [images.building3, 3.5, 'https://www.prulmarket.com/2PfkYA2Mg9/home', 'CHECK THE MARKET', `Sebuah acara tidak akan besar dan sukses tanpa adanya dukungan dari tim humas! Ya, SCUAD dengan bangga berkolaborasi dengan PRUL Channel dan PRUL Market, sebuah marketplace khusus barang-barang hobi yang unik dari Jepang. Kalau kamu tertarik dengan konten seputar jalan-jalan virtual di Jepang atau unboxing barang-barang mainan dari Jepang, jangan lupa cek YouTube PRUL Channel dan aplikasi PRUL Market ya!`, `イベントで大きなインパクトを出すには広報がかかせません。SCUADは現地メディアパートナーとして日本のホビーアイテムを扱う大手マーケットプレイスPrul MarketとPrul Channelにサポートいただいています。`], [images.building4, -3.5, 'https://future.nec/en/', 'LEARN MORE', `Bagaimana SCUAD bisa membuat acara yang mengundang para penyanyi ternama Indonesia? Tentu saja kami tidak sendiri. Kami bersyukur acara SCUAD ini disponsori oleh NEC. Apakah kamu familiar dengan istilah “Inovasi dari Jepang”? Yuk cek website mereka dan cari tahu tentang masa depan teknologi! `, `この挑戦的なイベントは、NEC未来創造会議や個人スポンサーの皆さまによって叶えることが出来ました。インドネシアの未来もわくわくさせる”日本発イノベーション”のこの先はこちらから。（英語ページが開きます。日本語へはリンク先で切り替えて下さい）`], [images.building5, 2, 'https://www.youtube.com/watch?v=LjQDIWmS7a0', 'WATCH', `Ide-ide cemerlang dari para SCUADers akan dinilai oleh juri-juri keren di SCUAD. Kami mengucapkan terima kasih atas dukungan dari KBRI Tokyo, Garuda Indonesia perwakilan Tokyo, dan tentunya Disbudpar Kota Bandung sebagai juri dalam acara ini!`, `バンドン市の観光と街づくりをテーマにした素敵な未来を作るアクションのジャッジとして、在日本インドネシア大使館、ガルーダエアライン東京社、バンドン市観光局にお越しいただきました。SCUADersたちのアイディアが、バンドン市から世界を変える種となりますように。`], [images.building6, -3, 'https://scuad.myshopify.com/', 'CHECK THE SHOP', `Save the moment! Miliki merchandise SCUAD sekarang juga sebelum kehabisan!`, `さぁ、SCUADのイベントです！イベントグッズを着てSCUADのメッセージを広めながら楽しくて素敵な未来を作っていきましょう！`], [images.gate, 0, null, null, null]].map(([imageInfo, x, link, button, messageId, messageJp], index) => new Building(imageInfo, x, index + 1, link, button, messageId, messageJp));
  ...
  const Event = new class {
    ...
    onTouchEnd(event) {
      if (!this.playable) return;
      this._touching = false; // this.roundPosition();
    }
